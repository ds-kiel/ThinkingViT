...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False, sparse_ratio=None, pruning_loc=None):
...
class AdaSwinTransformer(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, sparse_ratio=[0.9,0.7,0.5], pruning_loc=[4,8,12], **kwargs):
        super().__init__()
        ...
        for i_layer in range(self.num_layers):
            if i_layer == 2:
                layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               ...
                               use_checkpoint=use_checkpoint, sparse_ratio=sparse_ratio, pruning_loc=pruning_loc)
            else:
                layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               input_resolution=(patches_resolution[0] // (2 ** i_layer),
                                                 patches_resolution[1] // (2 ** i_layer)),
    ...
    def forward_features(self, x):
        ...
        featmap = self.norm(x)  # B L C
        x = self.avgpool(featmap.transpose(1, 2))  # B C 1
        x = torch.flatten(x, 1)

        return x, featmap, final_decisions

    def forward(self, x):
        x, featmap, decisions = self.forward_features(x)
        x = self.head(x)
...
class SwinTransformer_Teacher(nn.Module):
    ...
    def forward(self, x):
        x, featmap = self.forward_features(x)
        x = self.head(x)
        return x, featmap
