...
def train_one_epoch(model: torch.nn.Module, criterion,
    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if isinstance(logits_for_acc, (list, tuple)):
            logits_for_acc = logits_for_acc[0]
            if isinstance(logits_for_acc, (list, tuple)):
                logits_for_acc = logits_for_acc[-1]
        if mixup_fn is None:
            class_acc = (logits_for_acc.max(-1)[-1] == targets).float().mean()
        else:
            class_acc = None
