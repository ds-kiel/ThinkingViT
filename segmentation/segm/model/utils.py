...
def inference(
    ...
    return_stage_counts=False,
):
    C = model.n_cls
    seg_map = torch.zeros((C, ori_shape[0], ori_shape[1]), device=ptu.device)
    collect_stage_stats = (
        return_stage_counts and thinking_stages and thinking_threshold is not None
    )
    stage_hist = None
    if collect_stage_stats:
        stage_hist = torch.zeros(len(thinking_stages), dtype=torch.long, device=ptu.device)
        ...
        with torch.no_grad():
            for i in range(0, B, WB):
                ...
                if isinstance(model_out, (list, tuple)):
                    seg_maps[i : i + WB] = model_out[0]
                    if collect_stage_stats and len(model_out) > 1 and model_out[1] is not None:
                        stage_mask = model_out[1]
                        if stage_mask is not None:
                            stage_mask = stage_mask.to(ptu.device)
                            counts = torch.bincount(
                                stage_mask, minlength=stage_hist.numel()
                            )
                            stage_hist[: len(counts)] += counts[: stage_hist.numel()]
                else:
                    seg_maps[i : i + WB] = model_out
    ...
    if return_stage_counts:
        if stage_hist is not None:
            return seg_map, stage_hist.cpu().tolist()
        return seg_map, None
    return seg_map
