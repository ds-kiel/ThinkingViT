...
def _parse_stage_flops(stage_str):
    if stage_str is None:
        return None
    values = [v.strip() for v in stage_str.split(",")]
    result = []
    for value in values:
        if not value:
            continue
        try:
            ...
            raise click.BadParameter(
                f"Invalid stage FLOPs value '{value}'. Expected numeric entries."
            ) from exc
...
def process_batch(
    ...
    if collect_stage_counts:
        seg_pred, stage_counts = inference(
            model_without_ddp,
            ims,
            ims_metas,
            ori_shape,
    ...
    return filename, im.cpu(), seg_pred.cpu(), stage_counts
...
def eval_dataset(
    ...
    seg_pred_maps = {}
    idx = 0
    collect_stage_counts = thinking_threshold is not None and thinking_stages
    stage_totals = None
    if collect_stage_counts:
        stage_totals = torch.zeros(len(thinking_stages), dtype=torch.long)
    for batch in logger.log_every(db, print_freq, header):
        colors = batch["colors"]
        filename, im, seg_pred, stage_counts = process_batch(
            model,
            batch,
            window_size,
            window_stride,
            ...
            thinking_stages=thinking_stages,
            thinking_threshold=thinking_threshold,
            collect_stage_counts=collect_stage_counts,
        )
        ims[filename] = im
        seg_pred_maps[filename] = seg_pred
        if collect_stage_counts and stage_counts is not None:
            stage_counts_tensor = torch.tensor(stage_counts, dtype=torch.long)
            stage_totals += stage_counts_tensor[: stage_totals.numel()]
        idx += 1
    ...
    if save_images:
        label_suffix = f"_{result_label}" if result_label else ""
        save_dir = model_dir / f"images{label_suffix}"
        if ptu.dist_rank == 0:
            if save_dir.exists():
    ...
    if collect_stage_counts and stage_totals is not None:
        stage_totals_device = stage_totals.to(ptu.device)
        if ptu.distributed:
            torch.distributed.all_reduce(stage_totals_device)
        stage_totals = stage_totals_device.cpu()
    ...
    if ptu.dist_rank == 0:
        ...
        scores["thinking_threshold"] = thinking_threshold
        scores["thinking_stages"] = thinking_stages
        suffix = f"{suffix}_{result_label}" if result_label else suffix
        scores["cat_iou"] = np.round(100 * scores["cat_iou"], 2).tolist()
        if collect_stage_counts and stage_totals is not None:
            stage_counts_list = [int(v) for v in stage_totals.tolist()]
            total_decisions = sum(stage_counts_list)
            scores["stage_counts"] = stage_counts_list
            if total_decisions > 0:
                stage_portions = [v / total_decisions for v in stage_counts_list]
                scores["stage_portions"] = stage_portions
                print(f"Stage counts: {stage_counts_list}")
                print(
                    f"Stage portions: {[round(p, 4) for p in stage_portions]}"
                )
                if stage_flops:
                    avg_flops = float(np.dot(stage_portions, stage_flops))
                    scores["avg_flops"] = avg_flops
                    scores["stage_flops"] = stage_flops
                    print(f"Stage FLOPs: {stage_flops}")
                    print(f"Average FLOPs (weighted): {avg_flops:.4f}")
            elif stage_flops:
                print("Stage counts not available; skipping FLOPs aggregation.")
        for k, v in scores.items():
            if k not in ("cat_iou", "inference") and hasattr(v, "item"):
                scores[k] = v.item()
            if k != "cat_iou":
                print(f"{k}: {scores[k]}")
...
@click.option(
    ...
    type=str,
    default=None,
    help="Comma-separated head counts per thinking stage (overrides checkpoint).",
)
@click.option(
    "--thinking-threshold",
    type=str,
    ...
    help="Entropy threshold(s) for gating. "
         "Provide multiple values or 'none' to disable gating.",
)
@click.option(
    "--stage-flops",
    type=str,
    default=None,
    help="Comma-separated cumulative FLOPs (same order as thinking stages).",
)
def main(
    model_path,
    dataset_name,
    ...
    stage_flops_list = _parse_stage_flops(stage_flops)
    if stage_flops_list is not None:
        if not stage_list:
            raise click.BadParameter(
                "--stage-flops requires thinking stages to be specified."
            )
        if len(stage_flops_list) != len(stage_list):
            raise click.BadParameter(
                f"--stage-flops expects {len(stage_list)} values to match the stage list."
            )
    thresholds = _parse_thresholds(thinking_threshold, variant_thinking.get("threshold"))

    for threshold in thresholds:
        label = "no_thresh" if threshold is None else f"thr{threshold:.3f}"
        print(f"\n=== Evaluating threshold: {label} ===")
        eval_dataset(
            model,
            multiscale,
            model_dir,
