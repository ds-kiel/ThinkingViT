...
class SwinTokenProjector(nn.Module):
    """Upsample Swin stage-1 tokens via ConvTranspose2d, then reduce channels with 1x1 Conv2d."""
...
parser.add_argument('--thinking-debug', action='store_true', default=False,
                    help='print thinking-stage diagnostics (shapes, head usage) during forward passes')
...
def main():
    ...
    if hasattr(model, 'head_rounds') and len(model.head_rounds) == 2:
        ...
        thinking_schedule = [3, 6]
        model.alpha = nn.ParameterList([nn.Parameter(torch.zeros(1, device=device)) for _ in range(12)])
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            stage_heads = list(getattr(args, 'thinking_round_outputs', []))
            if epoch == 0 and loader_eval is not None and stage_heads:
                RLDropping(model, used_heads=int(stage_heads[-1]), ema=False, stage_idx=len(stage_heads) - 1)
                validate(
                        model,
                        loader_eval,
                        validate_loss_fn,
                        ...
                        log_suffix=f' -{stage_heads[-1]} HEADS (warmup)',
                )
            ...
            if loader_eval is not None:
                stage_heads = list(getattr(args, 'thinking_round_outputs', []))
                if not stage_heads:
                    stage_heads = [getattr(model, 'max_heads', 0)]
                stage_order = list(range(len(stage_heads) - 1, -1, -1))

                eval_metrics = None
                for order_idx in stage_order:
                    heads = int(stage_heads[order_idx])
                    RLDropping(model, used_heads=heads, ema=False, stage_idx=order_idx)
                ...
                if model_ema is not None and not args.model_ema_force_cpu:
                    ...
                    for order_idx in stage_order:
                        heads = int(stage_heads[order_idx])
                        RLDropping(model_ema, used_heads=heads, ema=True, stage_idx=order_idx)
                        ema_metrics = validate(
                            ...
                            device=device,
                            amp_autocast=amp_autocast,
                            log_suffix=f' (EMA)-{heads} HEADS',
                        )
                        if order_idx == stage_order[0]:
...
def RLDropping(model, used_heads: int, ema: bool, stage_idx: Optional[int] = None) -> None:
    if ema:
        target = model.module if hasattr(model, 'module') else model
    elif isinstance(model, NativeDDP):
        target = model.module
    else:
        target = model

    if hasattr(target, 'head_rounds'):
        schedule = [rounds[-1] for rounds in target.head_rounds]
        if stage_idx is None and used_heads in schedule:
            stage_idx = schedule.index(used_heads)
        if stage_idx is not None:
            target.eval_stage_index = stage_idx
        target.max_heads = max(schedule)
        target.p = used_heads / target.max_heads
        return

    max_heads = getattr(target, 'max_heads', None)
    if not max_heads:
        raise AttributeError('Model must define max_heads for RLDropping to operate.')

    target.p = used_heads / max_heads
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        def _forward():
            ...
            with amp_autocast():
                #----**----#
                outputs = model(input, threshold=None)
                if isinstance(outputs, (tuple, list)) and len(outputs) == 2:
                    loss = 0.5 * loss_fn(outputs[0], target) + 0.5 * loss_fn(outputs[1], target)
                else:
...
def validate(
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            with amp_autocast():
                output = model(input, threshold=None, train_val=True)
