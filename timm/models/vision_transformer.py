...
#
# Modifications:
# - Adjusted the code to extract the nested subnetworks + the thinking approach
...
class Identity(nn.Module):
    ...
    def forward(self, input: Tensor, keep_shape) -> Tensor:
        return input
...
class Mlp(nn.Module):
    ...
    def forward(self, x, p):
        keep_shape = int(p * self.in_features)
        keep_shape_2 = int(p * self.hidden_features)
        keep_shape_3 = int(p * self.out_features)
        x = self.fc1(x, keep_shape, keep_shape_2)
        x = self.act(x)
        x = self.drop(x)
        x = self.fc2(x, keep_shape_2, keep_shape_3)
        x = self.drop(x)
        return x
...
class Linear(nn.Module):
    ...
    def forward(self, input: Tensor, keep_shape_in, keep_shape_out=-1) -> Tensor:
        if keep_shape_out== -1:
            keep_shape_out = keep_shape_in
        if keep_shape_in:
            if len(input.shape) == 2:
                return F.linear(input[:,0:keep_shape_in],
                         self.weight[0:keep_shape_out,0:keep_shape_in],
                         self.bias[0:keep_shape_out])
...
class QKVLinear(nn.Module):
    ...
    def forward(self, input: Tensor, keep_shape) -> Tensor:
        if keep_shape:
            self.out_features
            l1 = [i for i in range(keep_shape)]
            l2 = [i for i in range(int(self.out_features/3), int(self.out_features/3) + keep_shape)]
            l3 = [i for i in range(2*int(self.out_features/3), 2*int(self.out_features/3) + keep_shape)]
            return F.linear(input[:,:,0:keep_shape],
                     self.weight[l1+l2+l3, 0:keep_shape],
                     self.bias[l1+l2+l3])
...
class LayerNorm(nn.Module):
    ...
    def forward(self, input: Tensor, keep_shape) -> Tensor:
        if keep_shape:
            return F.layer_norm(input[:,:,0:keep_shape],
                                 (keep_shape,),
                                self.weight[0:keep_shape],
                                self.bias[0:keep_shape],
                                self.eps)
...
class Attention(nn.Module):
    ...
    def forward(self, x, p):
        ...
        #####
        if len(x.shape) == 2:
            keep_shape = int(p*self.dim)
        if len(x.shape) == 3:
            keep_shape = int(p*self.dim)
        #####
        
        # qkv = self.qkv(x, keep_shape).reshape(B, N, 3, self.num_heads, keep_shape // self.num_heads).permute(2, 0, 3, 1, 4)   
        qkv = self.qkv(x, keep_shape).reshape(B, N, 3, int(p*self.num_heads), self.head_dim).permute(2, 0, 3, 1, 4)        
        ...
        # x = (attn @ v).transpose(1, 2).reshape(B, N, keep_shape)
...
class LayerScale(nn.Module):
    ...
    def forward(self, x, keep_shape):
        return x.mul_(self.gamma[0:keep_shape]) if self.inplace else x * self.gamma[0:keep_shape]
...
class Block(nn.Module):
    ...
    def forward(self, x, p):
        ...
        if len(x.shape) == 3:
            keep_shape = int(p*self.dim)
        #####
        x = x[:, :,0:keep_shape] + self.drop_path1(self.ls1(self.attn(self.norm1(x, keep_shape), p), keep_shape))
        x = x[:, :,0:keep_shape] + self.drop_path2(self.ls2(self.mlp(self.norm2(x, keep_shape), p), keep_shape))
        return x
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        # self.num_prefix_tokens += self.num_prompt_tokens # for prompt
        ...
        # self.prompt_token = nn.Parameter(torch.zeros(1, self.num_prompt_tokens, embed_dim))
    ...
    def forward_features(self, x, p, x_out_prev = None, thinking_stages=None) -> torch.Tensor:#-----#
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            for i , blk in enumerate(self.blocks):
                x = blk(x, p) #-----#
        x = self.norm(x, int(p*self.embed_dim)) #-----#
        return x
    ...
    def entropy(self, logits):
        ...
        entropy = Categorical(probs=topk).entropy().unsqueeze(1)
        return entropy
