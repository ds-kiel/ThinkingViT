...
def validate(args):
    ...
    if hasattr(model, 'head_rounds') and len(model.head_rounds) == 2:
        ...
        model.proj_layer = nn.Linear(64 * 3, 768).to(device)
    ...
    with torch.no_grad():
        ...
        with amp_autocast():
            model(input, threshold=0)

        end = time.time()

        stage_heads = []
        if hasattr(model, 'head_rounds'):
            stage_heads = [rounds[-1] for rounds in model.head_rounds]
        num_stages = max(len(stage_heads), 2)
        c_stage = [0 for _ in range(num_stages)]
        correct_stage = [0 for _ in range(num_stages)]
        total_stage = [0 for _ in range(num_stages)]
        ...
        for batch_idx, (input, target) in enumerate(loader):
            ...
            # compute output
            with amp_autocast():
                output, mask = model(input, threshold=args.threshold)
                mask = mask.view(-1)
                for stage_idx in range(num_stages):
                    count = (mask == stage_idx).sum().item()
                    c_stage[stage_idx] += count
            ...
            for stage_idx in range(num_stages):
                indices = (mask == stage_idx).nonzero(as_tuple=True)[0]
                if indices.numel() == 0:
                    continue
                preds = predictions_batch[indices]
                targets = target[indices]
                correct_stage[stage_idx] += (preds == targets).sum().item()
                total_stage[stage_idx] += targets.size(0)
        ...
        for idx in range(num_stages):
            dispatched = c_stage[idx]
            acc = 100. * correct_stage[idx] / total_stage[idx] if total_stage[idx] > 0 else 0
